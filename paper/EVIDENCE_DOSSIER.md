...
7. **New identity required:** a corrected run must publish a new checkpoint identity, dataset/evaluation manifest, config hash, metric report, and receipt graph. It must use cache schema 3 with an explicit token-layout-derived `center_token`, a canonical FP32 state representation, and provenance that distinguishes logical compute dtype from physical encoding; include the local pure-DNA tokenizer and canonicalization implementation in the Carbon runtime identity; and prove network-independent tokenization. Cache schema 2 is labeled replay-only and cannot support corrected training or rollout evidence. Never overwrite or relabel `v0.2.1-r1` as corrected.
...
- **Historical cache-v1 flaw and corrected contract:** cache v1 omitted `center_token`, so the same sequence pooled around two loci could collide. Cache schema 3 keys `(window_hash, encoder_hash, state_layer, pool_type, pool_radius, center_token, dtype)`, stores canonical FP32 post-pooling/pre-normalization states, records physical encoding separately, and uses digest-derived encoder/contig path components. Its provenance-aware index can coexist with schema 2 for the same logical key, but corrected reads require v3; schema 2 remains explicit replay-only compatibility. [POST-RELEASE VERIFIED]
...
- Corrected cache v3 uses immutable on-disk Parquet shards per (chrom × stride_block), keyed by `(window_hash, encoder_hash, state_layer, pool_type, pool_radius, center_token, dtype)`, with schema/physical provenance in a constrained SQLite index.
- Live pooling and normalization produce canonical IEEE-754 FP32 states. V3 cache embeddings are raw post-pooling/pre-normalization `fixed_size_list<float32>` values; normalization is applied after lookup and canonicalized again. Historical v2 `list<float16>` shards are replay-only.
- Cross-process publication is serialized. Shards are staged, reopened, schema/content verified, installed without clobbering an existing winner, and durably synced before one direct SQLite transaction records their rows. Full index rebuilds use a private validated database and atomic replacement.
- Encoder ID and contig path components are fixed ASCII SHA-256 digests, so separators, Unicode normalization, case, reserved names, and trailing punctuation cannot alias. Namespace traversal rejects symlinks and uses no-follow dirfd operations where supported.
- Race-resistant cache I/O is supported on Linux and macOS and fails closed on Windows or on runtimes without secure directory-descriptor/no-follow primitives. There is no path-only publication fallback. A durable one-publication intent recovers the only possible link/index crash gap without an append-time shard scan, and the first SQLite index is privately built before atomic exposure.
- This contract is a correctness foundation only. It does not claim that a full cache build, corrected training run, rollout benchmark, remote receipt set, or H200 execution has completed.
