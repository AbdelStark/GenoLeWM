...
14. For model/demo releases: stage the aggregate measured metrics as
    ...
    outputs by package-local path. The `release_inputs` row MUST record
    checked metrics artifact paths, efficiency input identities, and
    suite output identities from the validated payloads. The readiness
    report MUST carry measured efficiency latency, throughput, peak
    memory, runtime/sample context, limitations, sanitized efficiency
    command provenance, and row-derived `readiness` plus `blockers`
    entries with issue refs.
