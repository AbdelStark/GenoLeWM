...
14. For model/demo releases: stage the aggregate measured metrics as
    ...
    include every metrics JSON artifact consumed by readiness. The
    `release_inputs` row MUST record checked metrics artifact paths,
    efficiency input identities, and suite output identities from the
    validated payloads. The readiness report MUST carry measured
