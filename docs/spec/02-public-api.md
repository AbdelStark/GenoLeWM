...
Released clean-machine demos replay the published checkpoint and Carbon
artifact paths for their exact terminal-demo inputs; new runtime surfaces
still need artifact-backed validation before docs can extend that claim.
...
| `geno-lewm-rollout` | Implemented alpha | aggregate measured latent rollout state rows into eval-compatible cosine, L2, Recall@k, naive-baseline, and per-K stratification metrics; the June 8 and June 9 v0.2 readiness runs generated release-shaped rollout rows through the suite path | RFC-0004, RFC-0007 |
