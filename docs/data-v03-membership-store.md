...
| `lookup.sqlite` | Read-only point/range index and validation/evaluation iterator |

The manifest schema is
`configs/data_v03/membership-store.schema.json`. Build inputs use
`configs/data_v03/membership-build-spec.schema.json`; both reject unknown
fields.
...
23 local staged Parquet files
        │
        ├── exact byte/schema/count checks
        ...
        └── atomic Parquet + SQLite + manifest publication
```
...
The semantic identity deliberately excludes container-format bytes so the same
row contract is not redefined by a PyArrow or SQLite encoding detail. The
manifest still binds the exact SHA-256 and size of both physical files. The
verifier hashes those bytes, scans Parquet and SQLite independently, recomputes
ordering/count/dedup/leakage invariants, and requires both semantic scans to
match the manifest.

## Consumption

`MembershipStore.open(path)` verifies once and opens `lookup.sqlite` read-only.
Point and interval queries use SQLite indexes. `iter_role("validation")` and
...
membership does not trigger a haplotype exclusion.

Opening with `verify=False` skips the expensive full file scan and checks the
manifest plus SQLite metadata only. Use that mode only after one trusted
...
uv run ruff check geno_lewm/data/membership_store.py \
  tools/data/v03_membership_store.py tests/unit/test_data_membership_store.py
uv run mypy --strict geno_lewm/data/membership_store.py \
