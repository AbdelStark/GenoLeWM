...
| #197 v0.2 benchmark readiness | `python -m tools.release.v02_benchmark_readiness --metrics-json ... --rollout-speed-report ... --efficiency-report ... --output ... --require-ok` | New coverage/provenance contract; `--require-ok` also requires non-fixture, package-relative release inputs; expected `ok=false` until broader benchmark rows and #42 rollout speed pass from measured artifacts |
