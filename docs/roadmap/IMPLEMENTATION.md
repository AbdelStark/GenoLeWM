...
| #165 results report | `geno-lewm-eval`; `geno-lewm-carbon-baseline`; `geno-lewm-eval-all --require-v02-vep-metrics --require-v02-rollout-metrics`; `geno-lewm-rollout`; `python -m tools.release.rollout_state_examples`; `python -m tools.release.rollout_state_rows`; `python -m tools.release.v02_benchmark_suite`; `python -m bench.inference --release-efficiency` | Completed for the narrow v0.1 release; the June 8 HF Jobs v0.2 run produced broader benchmark rows, Carbon-baseline deltas, rollout-fidelity rows, efficiency, suite, and readiness evidence from public or reproducibly staged inputs, and the June 9 #203 rerun applied the suite to the #202 checkpoint lineage |
| #197 v0.2 benchmark readiness | `python -m tools.release.v02_benchmark_readiness --metrics-json ... --rollout-speed-report ... --rollout-speed-scope-report ... --efficiency-report ... --suite-report ... --output ... --require-ok` | Completed by the June 8 HF Jobs report with `ok=true`: <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v02-autonomous/9bec68ad04f2787dd0dfdf42d116050061ad53f6/suite-r11/model/v0.2_benchmark_readiness_report.json>. The report records measured values/deltas, measured efficiency latency/throughput/memory, efficiency command provenance, suite output identities, row-derived readiness entries with issue refs, CI-bearing VEP metrics, rollout generation reports, and non-fixture release-input provenance; the K20 #42 speed target is explicitly re-scoped for v0.2 reporting and remains open for true speed-target closure |
| #203 paper-grade benchmark rerun | `tools/jobs/v02_suite_run.sh`; `python -m tools.release.v02_benchmark_suite --execute`; `python -m tools.release.v02_benchmark_readiness --require-ok --require-release-inputs` | Completed by the June 9 HF Jobs report with `ok=true`: <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/suite/model/v0.2_benchmark_readiness_report.json>. The paper framing is negative-results/systems artifact-chain evidence, not improved model quality; K20 remains open in #42 |
| #205 serious-completion paper | `python -m tools.release.paper_draft --serious-completion --suite-dir ... --planning-demo-dir ... --output ...`; `python -m tools.release.paper_package --serious-completion --suite-dir ... --planning-demo-dir ... --paper-path ...` | Completed by the generated paper <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/paper/paper.serious-completion.md> and verifier report with `ok=true`: <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/paper/paper_package_report.json>. The artifact preserves negative-results/systems framing, K20 #42 limitations, and weak #204 planning-demo behavior |
