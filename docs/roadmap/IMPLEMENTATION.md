...
| #165 results report | `geno-lewm-eval`; `geno-lewm-carbon-baseline`; `geno-lewm-eval-all`; `python -m bench.inference --release-efficiency` | Completed for the narrow v0.1 release; broader benchmark and rollout metrics remain open |
| #197 v0.2 benchmark readiness | `python -m tools.release.v02_benchmark_readiness --metrics-json ... --rollout-speed-report ... --efficiency-report ... --output ... --require-ok` | New coverage/provenance contract; expected `ok=false` until broader benchmark rows and #42 rollout speed pass from measured artifacts |
