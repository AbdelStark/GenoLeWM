...
"""``geno-lewm-prepare-gnomad`` — local gnomAD shard builder."""
...
from geno_lewm.data import prepare_gnomad_shard
...
def main(
    input_vcf: Annotated[
        Path | None,
        typer.Option("--input-vcf", help="Local gnomAD VCF or VCF.gz file."),
    ] = None,
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Dataset root; writes gnomad/<release>/variants.parquet."),
    ] = None,
    release: Annotated[str, typer.Option("--release", help="gnomAD release label.")] = "v4.1",
    min_af: Annotated[float, typer.Option("--min-af", help="Minimum global AF to keep.")] = 0.01,
    ...
    if input_vcf is None:
        raise InputError("prepare-gnomad requires --input-vcf")
    if output is None:
        raise InputError("prepare-gnomad requires --output")
