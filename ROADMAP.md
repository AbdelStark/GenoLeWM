...
| v0.2 benchmark readiness | `python -m tools.release.v02_benchmark_readiness --metrics-json ... --rollout-speed-report ... --efficiency-report ... --output ... --require-ok` reconciles measured eval, efficiency, and AR rollout speed artifacts into `v0.2_benchmark_readiness_report.json` | New v0.2 gate for [#197](https://github.com/AbdelStark/GenoLeWM/issues/197); expected `ok=false` until broader benchmark rows and the [#42](https://github.com/AbdelStark/GenoLeWM/issues/42) rollout speed target pass |
...
- `eval_config.effective.yaml` records the committed eval config plus
  ...
  `geno-lewm-eval-all` refreshes it while recording metrics inputs as
...
- `geno-lewm-eval-all --metrics-json ... --output-metrics ... --output-report ...`
  ...
  without rerunning benchmarks and records required
