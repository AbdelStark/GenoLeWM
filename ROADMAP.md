...
- `python -m tools.release.rollout_state_rows --examples-jsonl ... --model-dir ...`
  generates `geno-lewm-rollout-states` JSONL from supplied measured
  source/target/candidate latent examples and the manifest-backed action
  encoder/predictor; it records package-relative provenance but does not
  run Carbon encoding or construct held-out haplotypes;
