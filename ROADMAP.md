...
- a newly trained public v0.2 checkpoint/model release that improves on
  the v0.1 release lineage; the June 9 #203 rerun on the #202 lineage
  is measured but mostly negative;
- a model-quality-positive v0.2 dataset/model/paper package around the
  June 8 and June 9 benchmark evidence;
- performance regression gates beyond the measured June 8 and June 9
  HF Jobs benchmark/readiness runs;
- useful-planning behavior beyond the June 9 released-artifact planning
  demo;
- hosted package-publication completion for the first `0.2.1` Python
  package tag.
...
| v0.2 benchmark rerun on #202 lineage | `tools/jobs/v02_suite_run.sh` stages the #202 model/dataset artifacts plus public v0.2 benchmark inputs, executes the suite, AR rollout speed, accepted K20 scope report, and final readiness gate | Completed for [#203](https://github.com/AbdelStark/GenoLeWM/issues/203) by the June 9 HF Jobs report with `ok=true`: <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/suite/model/v0.2_benchmark_readiness_report.json>. The measured result supports a negative-results/systems artifact-chain framing, not an improved-results claim; K20 remains open in #42 |
| v0.2 serious-completion paper | `python -m tools.release.paper_draft --serious-completion --suite-dir .../suite --planning-demo-dir .../planning-demo --output paper.serious-completion.md` renders a generated paper from #203 suite/readiness identities plus the #204 planning-demo manifest; `python -m tools.release.paper_package --serious-completion ...` re-renders and verifies the paper against those artifacts, preserving negative findings, K20 #42 limitations, and weak planning-demo behavior | Completed for [#205](https://github.com/AbdelStark/GenoLeWM/issues/205) by the public generated paper <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/paper/paper.serious-completion.md> and verifier report with `ok=true`: <https://huggingface.co/abdelstark/geno-lewm-runs/resolve/main/geno-lewm-v021-strong-4f36eef-10k-r1/paper/paper_package_report.json>. This is negative-results/systems evidence, not a model-quality breakthrough or useful-planning claim |
| Source distribution | `python -m build`, `twine check`, and `python -m tools.release.check_sdist_assets dist/*.tar.gz` verify package metadata and the release-critical repo assets needed by the dataset, model, eval, efficiency, terminal-demo, clean-replay, and publication-evidence gates | `0.2.1` is the first package-release target; publish through the protected tag workflow after local sdist/wheel gates pass |
...
- `python -m tools.release.v02_benchmark_suite --manifest ... --output-report ...`
  ...
  output identities by package-local path plus SHA-256 and size, but not
  replacing the downstream metrics/readiness validators; the suite report
  records the manifest by package-local path plus SHA-256 and size
  identity; final release-input readiness must run after the executed
  suite report exists and pass it with `--suite-report`; a second-pass
  suite manifest can express that command with `readiness.suite_report`;
...
**Status:** completed for the narrow v0.1 release. The June 8 HF Jobs
v0.2 run produced broader ClinVar/Carbon-baseline, BRCA2/TraitGym,
rollout-fidelity, efficiency, suite, and `ok=true` readiness evidence;
the June 9 #203 rerun applied the same evidence path to the #202
checkpoint lineage and supports a negative-results/systems
artifact-chain framing. The #205 serious-completion paper artifact is
published with an `ok=true` verifier report, but it preserves the same
negative findings. Future work should package a new v0.2 model/data
release and close the true #42 K20 speed target instead of treating the
accepted v0.2 scope decision as speed evidence.
...
- planning with CEM; the pure solver core, `geno-lewm-plan` alpha CLI,
  and `bench.planning` pure-solver timing report exist, and the June 9
  #204 released-artifact planning demo recorded a weak `manifest_runtime`
  run (`best_distance=23.656930390534644`, 384 evaluations, patience
  stop). Stronger useful-planning behavior and M3 Max/H100 performance
