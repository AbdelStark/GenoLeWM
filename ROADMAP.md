...
| Evaluation and efficiency | `geno-lewm-eval`, `geno-lewm-carbon-baseline`, `geno-lewm-eval-all --require-v02-vep-metrics --require-v02-rollout-metrics`, `geno-lewm-rollout`, `python -m tools.release.v02_benchmark_suite`, and `python -m bench.inference --release-efficiency` generate or orchestrate metrics, report, effective eval config, aggregate VEP/rollout metric coverage checks, and efficiency evidence | Completed for the narrow v0.1 release ([#165](https://github.com/AbdelStark/GenoLeWM/issues/165)); broader GenoLeWM-vs-Carbon deltas, real rollout state-row artifacts, and benchmark gates remain open |
