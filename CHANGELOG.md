...
- Added fail-closed `geno-lewm-export --target` and `--quantization`
  request validation so ONNX, Core ML, GGUF, int8, and int4 requests
  error explicitly until their target-specific implementations land.
